...
// --- HEARTBEAT LOGIC ---
//const sendHeartbeat = () => {
    //const hostel = localStorage.getItem('vgn_blood') || "FIELD STATION"; // Re-using your existing storage keys
    //const staffId = localStorage.getItem('vgn_allergies') || "UN-STAFF-ID";
    
    //statusMsg.innerHTML = `<p style="color: #00ADEF; font-weight: bold;">📡 Sending Heartbeat Check-in...</p>`;

    //navigator.geolocation.getCurrentPosition((pos) => {
        //const mapUrl = `http://maps.google.com/?q=${pos.coords.latitude},${pos.coords.longitude}`;
        //const smsBody = `💙 UNICEF HEARTBEAT\nSTAFF: ${staffId}\nLOC: ${hostel}\nGPS: ${mapUrl}\nSTATUS: ALL CLEAR.`;
        
        // Directly trigger the SMS for check-in (Single tap)
        //const contact1 = localStorage.getItem('vgn_contact') || "";
        //if (contact1) {
          //  window.location.href = smsUrlBuilder(contact1, smsBody);
        //}
      //  statusMsg.innerText = "Check-in Sent!";
    //    setTimeout(() => { statusMsg.innerText = "Sentinel-Care Ready"; }, 3000);
  //  });
//};

// --- ADD TO YOUR LISTENERS ---
// Add a separate small button in your HTML for this, or use a "Short Click" logic:
//const hbButton = document.getElementById('heartbeatButton');
//if (hbButton) {
  //  hbButton.addEventListener('click', sendHeartbeat);
//}
...
if(contact2Input) {
    ...
    const cancelSOS = () => {
        ...
        statusMsg.innerText = "UNICEF Sentinel-Care Ready";
    };
